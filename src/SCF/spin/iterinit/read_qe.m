function [mol, options]=read_qe(type,qepath)
% Read in structure information, charge density and wave function of QE.
% type = 'Molecule'/'Crystal', which is the output type of mol
% qepath is the absolute path of the xml and hdf5 files
...
% % RHO_OLD
% % rho_old is divided into nspin parts, the codes in QE has been modified to
% % get rho_old
% chargename=[qepath,'/charge-density-old.hdf5'];
% if nspin == 1
%     millg=h5read(chargename,'/MillerIndices');
%     millg=millg.';
%     nl=mill2nl(millg,n1,n2,n3);
...
%     drho= real(ifftn(drhog3d)*n1*n2*n3);  
%     %rho is stored by the format of up and down density
%     rho_old{1} = (arho+drho)/2;
%     rho_old{2} = (arho-drho)/2;
% elseif nspin == 4
%     rho_old = cell(4,1);
%     %arho = rhoup + rhodw
%     millg=h5read(chargename,'/MillerIndices');
