function [x, info] = isdf_stable_right_solve(a, b, options)
%ISDF_STABLE_RIGHT_SOLVE Solve x*b = a with an ISDF-aware fallback.
...
if isfield(options, 'ill_conditioned_solver') && ...
    ...
    if info.ill_conditioned && ~options.warn_ill_conditioned
        ...
        cleanup = onCleanup(@() restore_warnings(warning_state_near, warning_state_sing));
    end
    x = a / b;
end
end
