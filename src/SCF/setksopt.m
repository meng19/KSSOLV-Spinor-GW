...
% SETKSOPT Create and/or alter KSSOLV options structure
%
%    options = setksopt('param1',value1,'param2',value2,...);
...
%
%    newoptions = setksopt(oldoptions,'param1',value1,'param2',value2,...);
...
%
%     ishybrid       bool                 Whether the hybrid functional
%   maxphiiter       positive integer     maximum number of outer iterations in hybrid 
%                                         functional calculations
%       phitol       positive float       convergence tolerance for outer iteration
%    exxmethod       'normal'             Method to calculate hybrid functional
%					 'qrcp'       
%					 'kmeans'
%        fixip       bool                 Whether the interpolation points are fixed in 
%                                         hybrid functional calculation when ISDF is used
% dynamic_samp       bool                 Whether use dynamic sampling in 
%                                         hybrid functional calculation when ISDF is used
%      aceconv       bool                 Whether perform Fourier convolution in ACE.
%         GACE       bool                 Whether use GACE for energy band calculation
...
allfields = {'verbose';'eigmethod';'maxscfiter';'maxdcmiter';...
             ...
             'factorOrbitals';'dfrank';'ncbands';'continue';...
             'ishybrid';'vexx';'maxphiiter';'phitol';...
             'exxmethod';'fixip';'dynamic_samp';'isdf_threshold';...
             'numisdf';'ind_mu';'rank';'samp';'seed';'weight';'init';...
			 'aceconv';'store_tensors';'x_gamma_extrapolation';'GACE'};
...
options.ishybrid = 0;
options.vexx = [];
options.maxphiiter = 30;
options.phitol = 1e-8;
options.exxmethod = 'ace';

% ISDF parameters 
% fixip and dynamic_samp are useful only for hybrid functional calculations
options.fixip = false; % this parameter must be 'false' when ind_mu is empty
options.dynamic_samp = false;
options.numisdf = 1;
options.ind_mu = [];
options.rank = 6;
options.samp = 'kmeans';
options.seed = 0;
options.weight = 'power';
options.power = 1;
options.init = 'random';
options.isdf_threshold = 1e-3;
if options.fixip == true && isempty(options.ind_mu)
	error('options.fixip must be false when ind_mu is not provided');
end
...
% Check validity of structure field contents.
...
switch field
    ...
    case {'scftol','dcmtol','cgtol','eigstol','betamix','factorOrbitals','phitol','isdf_threshold'} % real scalar
    ...
    case {'degree', 'dfrank', 'ncbands', 'numisdf', 'rank' }  % integer
        [validvalue, errmsg, errid] = nonNegInteger(field,value); % boolean integer
    case {'force','ishybrid','continue','aceconv','store_tensors', 'x_gamma_extrapolation','GACE',...
          'fixip','dynamic_samp'} 
        [validvalue, errmsg, errid] = nonNegInteger(field,value);
    case {'vexx','exxmethod','rank', 'ind_mu', 'samp', 'seed', 'weight', 'init', 'power'}
        validvalue = 1;
    otherwise
        validfield = false;  
        validvalue = false;
