...
for ispinor = 1:ctx.nspinor
    ...
    epsilon_progress(block, ...
        progress_work * 0.20 * (2 * ispinor - 1) / ...
        (2 * ctx.nspinor), ...
        sprintf('Eps q:%d ik:%d s:%d real', ...
...
epsilon_progress(block, progress_work * 0.90, ...
    sprintf('Eps q:%d ik:%d s:%d polar', ...
    block.iq, block.ik, block.ispin));
