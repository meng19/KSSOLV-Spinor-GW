...
config_file = fullfile(test_root, 'test', 'cases', 'si8_isdf.json');
[eps, sig] = gw_run_json(config_file);

assert(all(isfinite(sig.sig(:))), 'Si8 test produced non-finite sigma.');
fprintf('SI8_ISDF_OK: %d diagonal bands.\n', size(sig.sig, 1));
