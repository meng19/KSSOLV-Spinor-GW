...
if isfield(options, 'unclamped_recommended_rank') && ...
        options.unclamped_recommended_rank ~= options.recommended_rank
    recommendation = sprintf(['ceil(sqrt(%d*%d)*%.3g) = %d, ' ...
        'clamped to product-space maximum %d'], nleft, nright, ...
        options.rank_ratio, options.unclamped_recommended_rank, options.max_rank);
else
    recommendation = sprintf('ceil(sqrt(%d*%d)*%.3g) = %d', ...
        nleft, nright, options.rank_ratio, options.recommended_rank);
end
fprintf(['\nISDF %s space: %d interpolation points from %d real-space ' ...
    'grid points (%d left x %d right bands); recommended %s\n'], ...
    product_type, options.rank, ngrid, nleft, nright, recommendation);
end
