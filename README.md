...
| `projection_block_elements` | `2e6` | Maximum number of complex elements in the temporary projected-product tensor.  The projection uses one full matrix block automatically when it fits this limit; raise it only when sufficient CPU/GPU memory is available to reduce block-loop overhead. |
