...
if use_isdf
    left = isdf.real_components(block.wfnkq, block.fft.Nfft1, ...
        block.idx.kq, block.ispin, ctx.nspinor, block.valence_bands);
    right = isdf.real_components(block.wfnk, block.fft.Nfft2, ...
        block.idx.k, block.ispin, ctx.nspinor, block.conduction_bands);
    isdf_options = ctx.eps.isdf;
    contribution.gme = isdf.matrix_elements(left, right, ...
        block.idx.q, size(block.fft.Nfft1), isdf_options);
    gw_block_progress(block, progress_work * 0.65, ...
        sprintf('E q%d i%d s%d isdf', ...
...
for iv_local = 1:numel(block.valence_bands)
    ...
    for ic_local = 1:numel(block.conduction_bands)
        ...
        gw_block_progress(block, ...
            ...
            sprintf('E q%d i%d eden %d/%d v%d c%d', ...
            block.iq, block.ik, pair_count, npairs, iv, ic));
    end
end
end
