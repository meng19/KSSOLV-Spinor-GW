...
%   mos2_cases = {'mos2_222_cohsex'};  % or another case below
%   mos2_use_gpu = false;              % override eps/sig use_gpu
%   mos2_abs_tol = 1e-7;
%   mos2_rel_tol = 1e-10;

if ~exist('mos2_cases', 'var')
    mos2_cases = {'mos2_222_cohsex', 'mos2_222_fullfreq_gw', ...
...
switch case_id
    ...
    case 'mos2_222_fullfreq_gw'
        result = run_mos2_222_fullfreq_gw(use_gpu_override);
...
function result = run_mos2_222_fullfreq_gw(use_gpu_override)
[sys, options, syms] = read_qe_gw('.\example\qe_data\mos2_222_spinor', 1);
[sys, options] = gw_setup(sys, options);
...
sig_result = sigma(eps_result, sig_input, sys, options, syms);
result = collect_mos2_result('mos2_222_fullfreq_gw', sig_result, sys);
end
