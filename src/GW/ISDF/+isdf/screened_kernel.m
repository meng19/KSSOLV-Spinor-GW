function kernel = screened_kernel(screened, target_zeta_g, sigma_vcoul, ...
        cache_key)
...
if nargin < 3 || isempty(sigma_vcoul)
    sigma_vcoul = epsilon_vcoul;
end
epsilon_vcoul = epsilon_vcoul(:);
sigma_vcoul = sigma_vcoul(:);
...
if (~build_full_matrix && ...
        ...
        numel(sigma_vcoul) ~= numel(epsilon_vcoul)
...
if use_gpu
    ...
    if ~isa(sigma_vcoul, 'gpuArray')
        sigma_vcoul = gpuArray(sigma_vcoul);
    end
...
if build_full_matrix
    ...
    if isequal(epsilon_vcoul, sigma_vcoul) && isreal(epsilon_vcoul)
        right_projector = left_projector';
    else
        right_projector = screened.zeta_g' .* sigma_vcoul.';
    end
else
    ...
    if isequal(epsilon_vcoul, sigma_vcoul) && isreal(epsilon_vcoul)
        ...
        right_projector = screened.zeta_g' * ...
            (sigma_vcoul .* conj(target_zeta_g));
    end
end
